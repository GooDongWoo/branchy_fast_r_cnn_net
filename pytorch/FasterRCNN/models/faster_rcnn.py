...
class FasterRCNNModel(nn.Module):
    ...
    """
        ...
        self._stage2_region_proposal_network._rpn_conv1.eval()
        self._stage2_region_proposal_network._rpn_class.eval()
        self._stage2_region_proposal_network._rpn_boxes.eval()

        self._stage3_detector_network._classifier.eval()
        self._stage3_detector_network._regressor.eval()
    ...
    def train_step(self, optimizer, image_data, anchor_map, anchor_valid_map, gt_rpn_map, gt_rpn_object_indices,
                   gt_rpn_background_indices, gt_boxes):
        """
    ...
    """
        ...
        detector_regression_loss = detector.regression_loss(predicted_box_deltas=detector_box_deltas,
    ...
    def train_step_ee(self, optimizer, image_data, anchor_map, anchor_valid_map, gt_rpn_map, gt_rpn_object_indices,
                      gt_rpn_background_indices, gt_boxes):
        """
    Performs one training step on a sample of data.
    ...
    def train_step_ALL(self, optimizer, image_data, anchor_map, anchor_valid_map, gt_rpn_map, gt_rpn_object_indices,
                   gt_rpn_background_indices, gt_boxes):
        """
    Performs one training step on a sample of data.
    ...
    """
        ...
        # Stage 2: Generate object proposals using RPN
        rpn_score_map2, rpn_box_deltas_map2, proposals2 = self._stage2_region_proposal_network(
            feature_map=feature_map,
            image_shape=image_shape,  # each image in batch has identical shape: (num_channels, height, width)
            anchor_map=anchor_map,
            anchor_valid_map=anchor_valid_map,
        ...
        detector_classes2, detector_box_deltas2 = self._stage3_detector_network(
            feature_map=feature_map,
            proposals=proposals2
        )
        ...
        detector_regression_loss2 = detector.regression_loss(predicted_box_deltas=detector_box_deltas2,
                                                            y_true=gt_box_deltas2)

        total_loss = rpn_class_loss + rpn_regression_loss + detector_class_loss + detector_regression_loss + rpn_class_loss2 + rpn_regression_loss2 + detector_class_loss2 + detector_regression_loss2
        loss = FasterRCNNModel.Loss(
            rpn_class=rpn_class_loss.detach().cpu().item(),
            rpn_regression=rpn_regression_loss.detach().cpu().item(),
            detector_class=detector_class_loss.detach().cpu().item(),
    ...
    """
        ...
        assert len(positive_anchors) + len(
            negative_anchors) >= self._rpn_minibatch_size, "Image has insufficient anchors for RPN minibatch size of %d" % self._rpn_minibatch_size
        ...
        num_positive_samples = min(self._rpn_minibatch_size // 2,
    ...
    """
        ...
        box_delta_targets[:, 0:2] = (
                                                gt_box_centers - proposal_centers) / proposal_sides  # ty = (gt_center_y - proposal_center_y) / proposal_height, tx = (gt_center_x - proposal_center_x) / proposal_width
        box_delta_targets[:, 2:4] = t.log(
            gt_box_sides / proposal_sides)  # th = log(gt_height / proposal_height), tw = (gt_width / proposal_width)
        ...
        gt_box_deltas[:, 0, :] = t.repeat_interleave(gt_classes, repeats=4, dim=1)[:,
                                 4:]  # create masks using interleaved repetition, remembering to ignore class 0
        gt_box_deltas[:, 1, :] = t.tile(box_delta_targets, dims=(1,
                                                                 self._num_classes - 1))  # populate regression targets with straightforward repetition (only those columns corresponding to class are masked on)
    ...
    def _sample_proposals(self, proposals, gt_classes, gt_box_deltas, max_proposals, positive_fraction):
        ...
        class_indices = t.argmax(gt_classes,
                                 axis=1)  # (N,num_classes) -> (N,), where each element is the class index (highest score from its row)
