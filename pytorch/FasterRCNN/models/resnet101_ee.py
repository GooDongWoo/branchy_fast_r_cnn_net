...
class FeatureExtractor(nn.Module):
    def __init__(self, resnet):
        ...
        self.exit_threshold = t.tensor([0.8],
                                       dtype=t.float32).to(
            device)  # for fast inference  #TODO: inference variable(not constant 0.8) need to make parameter
    ...
    def _build_exits(self, resnet):
        ...
        ee1 = IntrFE(64)
        ee1.load_state_dict(state_dict=t.load('./InterFE.pth'), strict=False)
...
class ResNetBackbone(Backbone):
    def __init__(self, architecture):
        ...
        if architecture == "ResNet101":
            ...
            resnet_a.conv1.load_state_dict(state_dict=t.load('./conv1.pth'), strict=False)
            resnet_a.bn1.load_state_dict(state_dict=t.load('./bn1.pth'), strict=False)
            resnet_a.relu.load_state_dict(state_dict=t.load('./relu.pth'), strict=False)
            resnet_a.maxpool.load_state_dict(state_dict=t.load('./maxpool.pth'), strict=False)
            resnet_a.layer1.load_state_dict(state_dict=t.load('./layer1.pth'), strict=False)
            resnet_a.layer2.load_state_dict(state_dict=t.load('./layer2.pth'), strict=False)
            resnet_a.layer3.load_state_dict(state_dict=t.load('./layer3.pth'), strict=False)
            # resnet_a.InterFE.load_state_dict(state_dict=t.load('./InterFE.pth'), strict=False)
            resnet_a.layer4.load_state_dict(state_dict=t.load('./layer4.pth'), strict=False)
