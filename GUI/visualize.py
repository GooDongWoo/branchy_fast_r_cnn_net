...
def show_detections(output_path, show_image, image, scored_boxes_by_class_index, class_index_to_name):
    ...
    for class_index, scored_boxes in scored_boxes_by_class_index.items():
        ...
        # print(scored_boxes.shape)
        if ((class_index != 1) and (class_index != 2) and (class_index != 3) and (class_index != 4) and
                (class_index != 6) and (class_index != 7) and (class_index != 8) and (class_index != 10) and
                (class_index != 12) and (class_index != 13) and (class_index != 14) and (class_index != 17) and
                (class_index != 19)):
            for i in range(scored_boxes.shape[0]):
                ...
                _draw_text(image=image, text=text, position=(scored_box[1], scored_box[0]), color=color, scale=1.5,
                           offset_lines=-1)
