...
def train(model):
    ...
    ("best" if options.save_best_to else "final"), options.eval_split))  # evaluate final or best model on full dataset
    evaluate_ee(
        model=model,
        eval_data=eval_data,
...
def predict_all_ee(model, split):
    ...
    for sample in iter(dataset):
        output_path = os.path.join(dirname, os.path.splitext(os.path.basename(sample.filepath))[0] + ".png")
        predict_ee(model=model, image_data=sample.image_data, image=sample.image, show_image=False,
                   output_path=output_path)
...
if __name__ == "__main__":
    ...
    group.add_argument("--predict-all_ee", metavar="name", action="store", type=str,
                       help="Run inference on all images in the specified dataset split and write to directory 'predictions_${split}'")
    ...
    parser.add_argument("--eval-split", metavar="name", action="store", default="test",
    ...
    if options.train:
        ...
        predict_one(model=model, url=options.predict, show_image=True, output_path="predictions.png")
